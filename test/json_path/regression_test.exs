defmodule JSONPath.RegressionTest do
  ...
  test "slice with large number for start end negative step" do
    ...
    assert {:ok, ["fifth", "forth"]} == JSONPath.evaluate(root, query)
  end

  test "slice with negative step" do
    query = "$[3:0:-2]"
    root = ["first", "second", "third", "forth", "fifth"]
    assert {:ok, ["forth", "second"]} == JSONPath.evaluate(root, query)
  end
  ...
  test "slice with negative step on partially overlapping array" do
    ...
    assert {:ok, ["fifth"]} == JSONPath.evaluate(root, query)
  end
  ...
  test "slice with negative step only" do
    ...
    assert {:ok, ["fifth", "third", "first"]} == JSONPath.evaluate(root, query)
  end
  ...
  test "slice with open end and negative step" do
    ...
    assert {:ok, ["forth", "third", "second", "first"]} == JSONPath.evaluate(root, query)
  end
  ...
  test "notation with quoted escaped backslash" do
    ...
    assert {:ok, ["value"]} == JSONPath.evaluate(root, query)
  end
  ...
  test "expression with equals null" do
    ...
    assert {:ok, [%{"key" => nil}]} == JSONPath.evaluate(root, query)
  end
  ...
  test "expression with equals with root reference" do
    ...
    assert {:ok, [%{"key" => 42}]} == JSONPath.evaluate(root, query)
  end
  ...
  test "expression with negation and equals" do
    ...
    assert {:ok, [%{"key" => 1}, %{"key" => nil}]} == JSONPath.evaluate(root, query)
  end
  ...
  test "expression with value after recursive descent" do
    ...
    assert {:ok, expected} == JSONPath.evaluate(root, query)
  end
